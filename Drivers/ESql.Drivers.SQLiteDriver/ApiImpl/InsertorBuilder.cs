...
public class InsertorBuilder<T> : BaseDisposer, IInsertor<T> where T : class, new()
{
    ...
    private SQLiteCommand? _commander;
    ...
    public InsertorBuilder<T> AddESqlInstance(IESql sql)
    {
        Instance = sql;
        return this;
    }
    ...
    public int ExecuteNoneQuery()
    {
        ...
        {
            var bulkConnection = new SQLiteConnection(ConnectStrings);
            try
            {
                bulkConnection?.Open();
    ...
    public IInsertor<T> AddRange(List<T> list)
    {
        ...
        _table.TableName =
            (typeof(T).GetCustomAttributes(typeof(HelloAttribute), true).FirstOrDefault() as HelloAttribute)?.Name;
    ...
    public string ToSql()
    {
        if (_statement.IsNullOrEmpty())
        {
            ...
            var type = typeof(T);
            var tbName = (type.GetCustomAttributes(typeof(HelloAttribute), true).FirstOrDefault() as HelloAttribute)
                ?.Name;
